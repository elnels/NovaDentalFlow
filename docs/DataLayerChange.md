...
| 2026-07-03 | Removed extraneous HC6 text fields (Diagnóstico de Presunción, Estudios Auxiliares, Observaciones) — dropped `observaciones_hc6` from `clinical_details`, and `diagnostico_presuncion`/`estudios_auxiliares`/`observaciones` from `clinical_history`. Migration: `20260703175214`. |
| 2026-07-03 | Dark mode fixes for HC6 odontogram: replaced all shadcn CSS variable classes with hardcoded Tailwind dark colors across 10 component files. Card background set to `rgb(30,30,30)`. ColorLegend moved outside grid to prevent overflow. Regresar button style matched to Continuar. |
