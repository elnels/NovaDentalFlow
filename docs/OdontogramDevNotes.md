...
17. **Odontogram tab dark mode** — `fix/odontogram-tab-background` (commit `f19cff1`). Profile tab wrapper divs changed from `bg-muted` to `bg-gray-900 shadow-sm border-gray-700 rounded-lg border`; `isDarkMode={true}` on `<Odontogram>` and `<FloatingToothDetailsCard>`.
18. **Auto-enable Dientes temporales for minors** — `feat/odontogram-temp-teeth-default` (commit `f05f138`). `if (res.esMenor) setShowTemporaryTeeth(true)` in both `hc6-form.tsx` and `odontogram-tab.tsx` — no longer manual toggle for minor patients.
19. **No erupcionado status** — `fix/odontogram-status-not-erupted` (commit `1f2700b`). Added `not_erupted` to `statusOptions` in `FloatingToothDetailsCard.tsx` with `HelpCircle` icon + `#14b8a6` color. The type already supported it; the UI button was missing.
